...
async function main() {
  ...
  // Monitor page crashes and console errors
  hostPage.on('crash', () => log('💥 HOST PAGE CRASHED'));
  hostPage.on('pageerror', (err) => log(`💥 Host JS error: ${err.message?.slice(0, 100)}`));
  for (let i = 0; i < 3; i++) {
    playerPages[i].on('crash', () => log(`💥 PLAYER ${names[i]} PAGE CRASHED`));
    playerPages[i].on('pageerror', (err) => log(`💥 Player ${names[i]} JS error: ${err.message?.slice(0, 100)}`));
  }
